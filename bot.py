...
def alert(guid,user,alert_text=""):
	...
	max_alert = 10    # you can change it
				...
				if not sleeped:
					...
					if msg["type"]=="Text" and not msg["message_id"] in answered:
						...
						if data["type"]=="RemoveGroupMembers":
							...
							bot.sendMessage(target, f"🚨 کاربر {user} از گروه کیک شد .", msg["message_id"])
							# bot.deleteMessages(target, [msg["message_id"]])
						
						elif data["type"]=="AddedGroupMembers":
							user = bot.getUserInfo(data['peer_objects'][0]['object_guid'])["data"]["user"]["first_name"]
						#	bot.sendMessage(target, f"سلام {user} عزیز 🌹 \n• به گروه {name} خوش اومدی 😍", msg["message_id"])
							# bot.deleteMessages(target, [msg["message_id"]])
